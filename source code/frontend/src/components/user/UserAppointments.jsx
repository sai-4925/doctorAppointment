...
const UserAppointments = () => {
  ...
  const getUserAppointment = async () => {
    console.log(userid)
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/users/getuserappointments`, {
        headers: {
  ...
  const getDoctorAppointment = async () => {
    console.log(userid)
    try {
        const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/doctor/getdoctorappointments`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
        params: {
  ...
  const handleStatus = async (userid, appointmentId, status) => {
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/doctor/handlestatus`, {
        userid,
        appointmentId,
        status,
      },
  ...
  const handleDownload = async (url, appointId) => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/doctor/getdocumentdownload`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem("token")}`,
        },
        params: { appointId },
