...
const AdminDoctors = () => {
   ...
   const getDoctors = async () => {
      try {
const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/admin/getalldoctors`, {
            headers: {
   ...
   const handleApprove = async (doctorId, status, userid) => {
      ...
      try {
        const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/admin/getapprove`, { doctorId, status, userid }, {
            headers: {
               Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
         })
   ...
   const handleReject = async (doctorId, status, userid) => {
      console.log(doctorId, status, userid)
      try {
         const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/admin/getreject`, { doctorId, status, userid }, {
            headers: {
               Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
         })
